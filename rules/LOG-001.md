...
**Criteria:** Log records with `status` = `debug` are not observed in a production environment for longer than 14 days, where a production environment is defined by the value of `env` _Resource_ attribute.
